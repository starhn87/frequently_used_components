...
전역에서 모달을 사용할 수 있게 컨텍스트를 만들면서 '어떻게 활성화된 모달 이외의 영역을 클릭시 모달을 비활성화하는 옵션을 줄 수 있을까'를 고민하였습니다. 제가 생각해낸 방법은 openModal 함수의 2번째 인자로 mode를 주어서 이것으로 strictMode 상태를 변경할 수 있도록 하여 strictMode가 false일 경우에만 이벤트 발생을 처리하는 방식으로 해결하였습니다.
