...
토글 관련 요소들을 prop으로 받아 Toggle 안에서 mouseup 이벤트가 발생할 때마다 업데이트시킴으로써 해당 컴포넌트가 상태에 따른 동작을 할 수 있도록 구현하였습니다.
...
| `onClick`  | `function` |         | checked 변경시 동작할 함수   |
...
토글 스위치를 클릭하시면 스위치가 토글되며 토글 여부에 따라 아래의 텍스트도 같이 변경됩니다.
<br>
...
Modal 관련 요소들을 prop으로 받아 이벤트에 따라 Modal 활성화시킬 수 있고 활성화 여부에 따라 다르게 동작할 수 있도록 구현하였습니다.
...
| onChange     | function |        | value 변경시에 따른 동작 함수                                  |
...
prop으로 탭을 정의하는 객체 배열과 활성화된 탭의 key값, 활성화 탭 변경시 동작 함수를 받아서 동작하도록 하였습니다.
...
| onChange    | function | selectedTab 변경시 동작할 함수 |
...
태그 컴포넌트이므로 태그 상태와 그에 다른 동작 함수는 prop으로 받도록 하고 내부 기능 구현에 필요한 상태들은 내부에서 선언하여 따로 관리될 수 있게끔 구현하였습니다.

##### Props

| 이름     | 타입     | 상세                     |
| -------- | -------- | ------------------------ |
| value    | array    | 태그들이 담긴 배열 상태  |
| onChange | function | value 변경시 동작할 함수 |
...
데이터셋과 자동 완성될 배열 상태, 그에 따른 동작 함수를 prop으로 받고 input 입력값과 focus 여부는 내부적으로 선언하여 따로 관리하게끔 구현하였습니다.

##### Props

| 이름     | 타입     | 상세                           |
| -------- | -------- | ------------------------------ |
| value    | array    | auto complete된 정보 배열 상태 |
| onChange | function | value 변경시 동작할 함수       |
| options  | object   | 입력값과 매칭될 데이터셋       |
...
컴포넌트는 재사용 가능한 최소한의 단위이므로 input과 그 관련 속성들, 클릭시 값과 그 값 변경시 동작할 함수를 prop으로 받도록 하여 재사용할 수 있도록 구현하였습니다.
...
처음에는 prop으로 받은 value로만 구현하려고 했지만 그렇게 되면 입력값 변경시 실시간으로 아래의 텍스트 부분도 바로 반영되는 이슈가 있었습니다.
따라서 컴포넌트 내부적으로 입력값 상태 (editingValue)를 추가하여 포커스 해제시에만 value에 반영되도록 하였습니다.
