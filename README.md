...
전역으로 사용할 수 있도록 modal 컴포넌트와 컨텍스트를 작성하였습니다. 컨텍스트에서 인터페이스에 사용될 상태와 함수를 정의하고 모달 컴포넌트에 prop으로 넘기는 방식으로 구현하였습니다. 또한, 모달을 활성화시킬 수 있는 함수를 전역에서 접근할 수 있도록 하여 모달을 사용할 수 있도록 하였습니다.
...
| `onOutOfModalClick` | `function` | `false` | 모달 활성화 후 모달 이외의 부분 클릭시 동작할 함수 |
...
태그 컴포넌트이므로 필요한 상태와 함수들은 prop으로 받도록 하고 내부 기능 구현에 필요한 상태들은 내부에서 선언하여 따로 관리될 수 있게끔 구현하였습니다. 이미 존재하는 태그를 다시 추가시 기존의 모달을 재사용할 수 있도록 하였습니다.
