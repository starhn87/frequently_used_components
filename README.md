...
최상단에 컴포넌트를 선언한 후 DOM 안에서 어느 곳을 클릭하던지 해당 기능을 사용하실 수 있습니다.
<br>
...
토글 인터페이스에 필요한 요소들을 prop으로 받아 Toggle 안에서 클릭 이벤트가 발생할 때마다 업데이트시킴으로써 해당 컴포넌트가 상태에 따른 동작을 할 수 있도록 구현하였습니다.
...
토글 스위치 클릭시 색깔을 방향성 있게 채우고 비우게 하는 것에서 난관이 있었습니다.
그래서 css 속성을 검색하여 linear-gradient라는 속성을 추가하여 구현하였습니다.

#### 실행 방법

토글 스위치를 클릭하시면 스위치가 토글되며 On/Off 여부에 따라 아래의 텍스트도 같이 변경됩니다.
disabled를 true로 하면 토글 스위치가 작동하지 않게 됩니다.
<br>
...
전역으로 사용할 수 있도록 modal 컴포넌트와 함께 컨텍스트를 만들었습니다. 컨텍스트에서 인터페이스에 사용될 상태와 함수를 정의하고 모달 컴포넌트에 prop으로 넘기는 방식으로 구현하였습니다. 또한, 모달을 활성화시킬 수 있는 함수를 전역에서 접근할 수 있도록 하여 모달을 재사용할 수 있도록 하였습니다.

##### props

| 이름                | 타입       | 상세                                               |
| ------------------- | ---------- | -------------------------------------------------- |
| `closeModal`        | `function` | 모달 비활성화시 동작할 함수                        |
| `content`           | `string`   | 모달에서 보여줄 컨텐츠                             |
| `onOutOfModalClick` | `function` | 모달 활성화 후 모달 이외의 부분 클릭시 동작할 함수 |

#### 에러 로그

전역에서 모달을 사용할 수 있게 컨텍스트를 만들면서 '어떻게 활성화된 모달 이외의 영역을 클릭시 모달을 비활성화하는 옵션을 줄 수 있을까'를 고민하였습니다. 제가 생각해낸 방법은 openModal 함수의 2번째 인자로 mode를 주어서 이 모드로 strictMode 상태를 변경할 수 있도록 하여 strictMode가 false일 경우에만 이벤트 발생을 처리하는 방식으로 해결하였습니다.

#### 실행 방법

Open Modal 버튼 클릭시 모달이 보여지고 모달의 X 버튼 클릭시 모달이 사라집니다.
만약, 모달 이외의 부분 클릭시 모달이 사라지게끔 하고 싶다면 openModal 함수의 두 번째 인자 (mode)를 false로 설정하여 strictMode 상태를 false로 변경하면 됩니다.
아래의 예시는 mode를 false로 설정한 경우입니다.
<br>
...
인터페이스에 필요한 상태와 함수들은 prop으로 받도록 하고 내부 기능 구현에 필요한 상태들은 내부에서 선언하여 따로 관리될 수 있게끔 구현하였습니다. 이미 존재하는 태그를 다시 추가시 기존의 모달을 재사용할 수 있도록 하였습니다.
...
자동 완성 컴포넌트 인터페이스에 필요한 요소들을 prop으로 받고 active 여부는 내부적으로 선언하여 따로 관리하게끔 구현하였습니다.
